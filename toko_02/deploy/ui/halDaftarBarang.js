...
function item(barang) {
    ...
    barang.forEach((item) => {
        hasil += `<div class='disp-flex'>
            ...
            <div class='menu'>
                <a href="${Util_1.util.getUrl(store_1.URL.gudang_barang_edit, [item.id])}">edit info</a>
                <a href="${Util_1.util.getUrl(store_1.URL.gudang_barang_edit, [item.id])}">edit foto</a>
                <a href="#" onclick="hapus(${Util_1.util.getUrl(store_1.URL.gudang_barang_hapus, [item.id])})">hapus</a>
            </div>
        </div>`;
    });
    return hasil;
...
function menu() {
    return `
        <a href="${Util_1.util.getUrl(store_1.URL.gudang_barang_baru, [])}">tambah barang</a>
    `;
}
...
async function data() {
    let barang = await toko_1.toko.gudang.barang.daftar();
    return html(barang);
}
